...
- **逐回复标注行** — 每条最终回复下方显示一行灰色小字：该回复总耗时（用户消息 → 回复完成，含思考+生成+工具）与工具调用次数；随会话持久化（不进 LLM 上下文），重启后仍在
...
每条最终回复下方追加一行灰色小字：该回复总耗时（用户消息 → 回复完成）与工具调用次数。语言在追加时定格（跟随发起该回复的消息语言）。开关为内存态（重启恢复 on）；已写入的标注行保留在会话中。
